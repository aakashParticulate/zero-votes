...
* document AbstractFacade + RecipientFacade
...
2. __Poll states__
	...
	2.1 The system must implement poll states. Polls be in one of four states: PREPARED, STARTED, VOTING, FINISHED. State changes are specified below.
		(_When a poll is created, its state is PREPARING. When it is published it will be PUBLISHED. At the start time it's set to STARTED. After the first vote it's VOTING. At the end time it is set to FINSIHED._)
...
5. __Participants__
	...
	(_An organizer can add only at least 3 participants to a poll._)
...
6. __Participant lists__
    ...
    6.1 The organizer must be able to modify the participant list until a poll is STARTED. 
		(_The organizer must be able to modify the participant list until the poll is PUBLISHED._)
    
    6.2 The organizer must be able to modify the participant list until a poll is STARTED. 
		(_The system provides means to comfortably create participant lists by pasting a list of email addresses. KLIPS, SpreadSheets, are supported._)
    
    6.3 The system shall provide a means to store participant lists for easy reuse in subsequent polls.
		(_An organizer can store recipient lists for an easy reuse of participant lists._)
...
9. __Participation tracking__
    ...
    9.2 The system can provide an option to configure automatic reminders via E-Mail.
		(_If participation tracking is enabled a reminder email can be send to participants, who did not vote until this time._)
...
14. __User interface__
    ...
    14.1 The system can use third-party CSS libraries (such as Twitter Bootstrap) to achieve a modern look and feel.
		(_The system uses Twitter Bootstrap, Bootstrap-Progressbar, Toastr (used for messages), Boostrap-Datetimepicker, Font-Awesome (Icons), Highcharts (Charts)_)
...
16. __Internationalization__
    ...
    16.4 The language switch can be done by clicks to icons or links.
		(_The language switch is realised with a form select in the footer on the right side which autosubmits itself on selection change._)
